...
class ApplicationService {
    async applyToJob(userId, payload) {
        ...
        if (studentUser) {
            ...
            const companyName = jobWithCompany?.company ? jobWithCompany.company.companyName : 'the company';
            const emailContent = `
    ...
    async getById(applicationId, actor) {
    ...
    async updateStatus(applicationId, payload, actor) {
        ensureObjectId(applicationId, 'application id');
        const application = (await Application.findById(applicationId)
        ...
        const updated = await Application.findByIdAndUpdate(applicationId, { status: payload.status, decisionHistory: updatedHistory }, { new: true }).lean();
        ...
        if (studentUser) {
            ...
            const companyName = jobData?.company ? jobData.company.companyName : 'the company';
            let statusMessage = '';
            let alertType = 'info';
      ...
      `;
        ...
        if (payload.status === 'selected') {
            ...
            await Promise.all(admins.map(async (admin) => {
                ...
                const studentName = studentData?.user ? studentData.user.fullName : 'a student';
                const emailContent = `
            ...
            ${emailTemplate.bulletList([
                    `Student: ${studentName}`,
                    `Position: ${jobData?.title || 'N/A'}`,
                    `Company: ${jobData?.company ? jobData.company.companyName : 'N/A'}`,
                    `Package: ${jobData?.packageLpa ? `${jobData.packageLpa} LPA` : 'N/A'}`,
