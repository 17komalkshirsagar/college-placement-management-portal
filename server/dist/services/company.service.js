...
class CompanyService {
    async updateProfile(companyId, payload, actor) {
    ...
    async updateActivation(companyId, payload) {
        ensureObjectId(companyId, 'company id');
    ...
    async adminUpdateCompany(companyId, payload) {
        ensureObjectId(companyId, 'company id');
    ...
    async adminUpdateStatus(companyId, payload) {
        ensureObjectId(companyId, 'company id');
        const company = await CompanyProfile.findByIdAndUpdate(companyId, { isActive: payload.isActive }, { new: true }).lean();
        if (!company) {
            throw new ApiError(404, 'Company not found');
    ...
    async adminDeleteCompany(companyId) {
        ensureObjectId(companyId, 'company id');
    ...
    async listCompanies(query) {
        ...
        const isActive = typeof query.isActive === 'string' ? (query.isActive === 'true' ? true : query.isActive === 'false' ? false : undefined) : undefined;
        const filter = {};
